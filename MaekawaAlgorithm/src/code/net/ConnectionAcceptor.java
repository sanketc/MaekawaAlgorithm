		...
		try {
			while(true) {
				...
				if(request instanceof ConnectMessage) {
					...
					System.out.println("Connected to : " + request.getConfig().getId());
					connectionPoolTable.put(request.getConfig().getId(), con);
					
					// start listner thread
					System.out.println("Started listneing for : " + request.getConfig());
